                  ...
                  itemBuilder: (_, i) {
                    ...
                    return Container(
                      ...
                      child: InkWell(
                        ...
                        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ChatScreen(role: widget.role, conversation: c))).then((_) => _load()),
                        child: Padding(
                          padding: const EdgeInsets.all(10),
                          child: Row(children: [
                            logoOf(c) == null
                                ? CircleAvatar(child: Text((nameOf(c) ?? '؟').toString().characters.first, style: const TextStyle(fontWeight: FontWeight.w900)), backgroundColor: A.primaryLight.withOpacity(0.25))
                                : storeLogo('${logoOf(c)}', size: 42, radius: 13),
                            const SizedBox(width: 11),
                            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                              Text('${nameOf(c)}', style: A.t(14, w: FontWeight.w900)),
                              Text('${c['last_message'] ?? 'بداية المحادثة'}', maxLines: 1, overflow: TextOverflow.ellipsis, style: A.t(11.5, c: unread ? A.primary : A.muted, w: unread ? FontWeight.w800 : FontWeight.w400)),
                            ])),
                            if (unread) Container(width: 9, height: 9, decoration: const BoxDecoration(color: A.primary, shape: BoxShape.circle)),
                          ]),
                        ),
                      ),
                    );
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  String get _base => '/api/${widget.role}/conversations/${widget.conversation['id']}/messages';
      ...
      body: Column(children: [
        Expanded(
          child: loading
              ? const Loader()
              : messages.isEmpty
                  ? const EmptyState(icon: '👋', title: 'ابدأ المحادثة', sub: 'اسأل عن منتج أو مدة التوصيل')
                  : ListView.builder(
                        ...
                        return Align(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              color: mine ? A.primary : Colors.white,
                            ...
                            child: Text('${m['body']}', style: A.t(13.5, c: mine ? Colors.white : A.ink)),
                          ),
                        );
                      },
                    ),
