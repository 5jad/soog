...
String? storeMapLink(Object? lat, Object? lng, String? url) {
  ...
  if (lat != null && lng != null) {
    ...
    if (l.isNotEmpty && g.isNotEmpty) return 'https://www.google.com/maps/search/?api=1&query=$l,$g';
  }
  return null;
}

Future<void> openStoreMap(BuildContext ctx, Object? lat, Object? lng, String? url) async {
...
class _DeliveryShellState extends State<DeliveryShell> {
  ...
  Future<void> toggleOnline() async {
    try {
      ...
      toast(context, online ? 'صرت متصل — الطلبات توصلك الآن 🛵' : 'صرت متوقف عن الاستلام');
    } catch (_) {}
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: online ? A.success : A.bg,
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: online ? A.success : A.line),
              ),
              child: Row(children: [
                Icon(Icons.circle, size: 9, color: online ? A.success : A.muted),
      ...
      body: IndexedStack(
        index: tab,
        children: [
          _AvailableOrders(online: online, refresh: _load, onAccepted: () => setState(() => tab = 1)),
          _MyTrip(),
...
class _AvailableOrders extends StatefulWidget {
  ...
  const _AvailableOrders({required this.online, required this.refresh, this.onAccepted});
  @override
...
class _AvailableOrdersState extends State<_AvailableOrders> {
  ...
  Widget build(BuildContext context) {
    ...
    if (!widget.online) {
      return const EmptyState(icon: '😴', title: 'أنت غير متصل', sub: 'فعل زر "متصل" فوق عشان توصلك الطلبات');
    }
    if (orders.isEmpty) {
      return RefreshIndicator(
        onRefresh: _load,
        color: A.primary,
        child: ListView(children: const [SizedBox(height: 200), EmptyState(icon: '📡', title: 'لا طلبات متاحة', sub: 'رح ينزلون هنا لحظة ما جاهزون')]),
      );
    }
    return RefreshIndicator(
      onRefresh: _load,
        ...
        itemBuilder: (_, i) {
          ...
          return GlassCard(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                Expanded(child: Text('#${order.code}', style: A.t(14, w: FontWeight.w900))),
                Text('جاهز 🎒', style: A.t(11.5, c: A.cyan, w: FontWeight.w800)),
              ]),
              const SizedBox(height: 10),
              Row(children: [
                storeLogo(order.storeLogo, size: 34, radius: 10),
                const SizedBox(width: 8),
                Expanded(child: Text(order.storeName, style: A.t(12.5))),
                if (storeMapLink(o['store_lat'], o['store_lng'], o['store_location_url'] as String?) != null)
                  IconButton(
                    visualDensity: VisualDensity.compact,
                    icon: const Icon(Icons.navigation_rounded, size: 18, color: A.primary),
                    tooltip: 'موقع المتجر على الخريطة',
                    onPressed: () => openStoreMap(context, o['store_lat'], o['store_lng'], o['store_location_url'] as String?),
                  ),
                Text(money(order.total), style: A.t(14, c: A.accent, w: FontWeight.w900)),
              ]),
              const SizedBox(height: 8),
              Row(children: [
                const Icon(Icons.storefront_rounded, size: 15, color: A.primary),
                const SizedBox(width: 5),
                Expanded(child: Text('استلام من المتجر: ${o['store_address'] ?? ''}', style: A.t(11.5, c: A.muted), maxLines: 1, overflow: TextOverflow.ellipsis)),
              ]),
              const SizedBox(height: 5),
              Row(children: [
                const Icon(Icons.location_on_rounded, size: 15, color: A.muted),
                const SizedBox(width: 5),
                Expanded(child: Text(o['user_address_label'] ?? o['address_text'] ?? '', style: A.t(11.5, c: A.muted), maxLines: 1, overflow: TextOverflow.ellipsis)),
              ]),
                ...
                OutlinedButton(
                  ...
                  onPressed: () => Navigator.push(context, MaterialPageRoute(
                    builder: (_) => RoutePreviewScreen(
                      from: LatLng((o['store_lat'] as num).toDouble(), (o['store_lng'] as num).toDouble()),
                      to: LatLng((o['user_lat'] as num).toDouble(), (o['user_lng'] as num).toDouble()),
                      fromLabel: order.storeName,
                      toLabel: o['user_address_label'] as String? ?? 'الزبون',
                    ),
                  )),
                  child: const Text('شوف المسار والمسافة 🗺️', style: TextStyle(fontWeight: FontWeight.w800)),
                ),
              SolidBtn(
                label: 'استلم الطلب 🛵',
  ...
  Widget _groupCard(List<Map> g) {
    final total = g.fold<int>(0, (s, o) => s + ((o['total'] ?? 0) as num).toInt());
    LatLng? home;
    final stops = <LatLng>[];
    for (final o in g) {
      if (o['user_lat'] != null && o['user_lng'] != null && home == null) {
        home = LatLng((o['user_lat'] as num).toDouble(), (o['user_lng'] as num).toDouble());
      }
      if (o['store_lat'] != null && o['store_lng'] != null) {
        stops.add(LatLng((o['store_lat'] as num).toDouble(), (o['store_lng'] as num).toDouble()));
      }
    }
    return GlassCard(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Expanded(child: Text('طلب من ${g.length} محلات 🛍', style: A.t(14, w: FontWeight.w900, c: A.ink))),
          Text('جاهز 🎒', style: A.t(11.5, c: A.cyan, w: FontWeight.w800)),
        ]),
        const SizedBox(height: 4),
        Text('رحلة واحدة تاخذ من كل المحلات', style: A.t(10.5, c: A.muted)),
          ...
          Padding(
            ...
            child: Row(children: [
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Text(o['store_name'] ?? '', style: A.t(12, w: FontWeight.w800), maxLines: 1, overflow: TextOverflow.ellipsis),
                Text('#${o['code']} · ${money(o['total'] ?? 0)}', style: A.t(10.5, c: A.muted)),
              ])),
              if (storeMapLink(o['store_lat'], o['store_lng'], o['store_location_url'] as String?) != null)
                IconButton(
                  visualDensity: VisualDensity.compact,
        ...
        Row(children: [
          Text('المجموع ', style: A.t(12, c: A.muted)),
          Text(money(total), style: A.t(14, c: A.accent, w: FontWeight.w900)),
        ]),
        const SizedBox(height: 12),
          ...
          OutlinedButton(
            ...
            onPressed: () => Navigator.push(context, MaterialPageRoute(
              builder: (_) => RoutePreviewScreen(
                ...
                toLabel: g.first['user_address_label'] as String? ?? 'الزبون',
              ),
            )),
            child: const Text('شوف المسار والمسافة 🗺️', style: TextStyle(fontWeight: FontWeight.w800)),
          ),
  ...
  Widget build(BuildContext context) {
    ...
    if (trip == null || trip == '') {
      return RefreshIndicator(
        onRefresh: _load,
        color: A.primary,
        child: ListView(children: const [SizedBox(height: 200), EmptyState(icon: '🛵', title: 'ما عندك رحلة حالياً', sub: 'روح تبويب "متاح" واستلم طلب')]),
      );
    }
        ...
        children: [
          Container(
            ...
            child: Column(children: [
              Text('#$code', style: A.t(22, c: Colors.white, w: FontWeight.w900)),
              const SizedBox(height: 4),
              Text(statusAr(status), style: A.t(12.5, c: Colors.white70)),
              if (elapsed.isNotEmpty) ...[
                const SizedBox(height: 8),
                Text('⏱ $elapsed', style: A.t(12, c: Colors.white)),
              ],
            ]),
          ),
          const SizedBox(height: 14),
          SolidBtn(label: 'خريطة التوصيل الحية 🗺️', onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => CourierMapScreen(trip: t)))),
          const SizedBox(height: 14),
          if (((t['orders'] as List?) ?? []).length > 1) ...[
            GlassCard(
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(children: [
                  const Icon(Icons.storefront_rounded, color: A.primary, size: 18),
                  const SizedBox(width: 8),
                  Text('طلب من ${(t['orders'] as List).length} محلات 🛍', style: A.t(14, w: FontWeight.w900, c: A.ink)),
                ]),
                ...
                for (final o in t['orders'] as List)
                  Padding(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Text(shortName(o['store_name'] as String?), style: A.t(12.5, w: FontWeight.w800)),
                        Text('#${o['code']} · ${money((o['total'] ?? 0) as num)}', style: A.t(10.5, c: A.muted)),
                      ])),
                      IconButton(
                        visualDensity: VisualDensity.compact,
                        icon: const Icon(Icons.navigation_rounded, color: A.primary, size: 20),
                        tooltip: 'موقع المحل على الخريطة',
                        onPressed: () => openStoreMap(context, o['store_lat'], o['store_lng'], o['store_location_url'] as String?),
                      ),
                    ]),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                ...
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('$storeName', style: A.t(14, w: FontWeight.w900)),
                  Text('المتجر — الاستلام من هنا', style: A.t(10.5, c: A.muted)),
                ]),
                const Spacer(),
                IconButton(
                  visualDensity: VisualDensity.compact,
                  icon: const Icon(Icons.navigation_rounded, color: A.primary),
                  tooltip: 'موقع المتجر على الخريطة',
                  onPressed: () => openStoreMap(context, t['store_lat'], t['store_lng'], t['store_location_url'] as String?),
                ),
              ]),
              const Divider(height: 22),
              Row(children: [
                const Icon(Icons.storefront_rounded, color: A.primary, size: 17),
                const SizedBox(width: 8),
                Expanded(child: Text('$storeAddress', style: A.t(12.5))),
              ]),
            ]),
          ),
          const SizedBox(height: 14),
          GlassCard(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(children: [
                const Icon(Icons.person_rounded, color: A.primary, size: 18),
                const SizedBox(width: 8),
                Expanded(child: Text('$userName', style: A.t(13.5, w: FontWeight.w800))),
                Text('$userPhone', style: A.t(11.5, c: A.muted)),
              ]),
              const Divider(height: 20),
              Row(children: [
                const Icon(Icons.location_on_rounded, color: A.primary),
                const SizedBox(width: 8),
                Expanded(child: Text('$userAddress', style: A.t(13))),
                if (storeMapLink(t['user_lat'], t['user_lng'], null) != null ||
                    userAddress.isNotEmpty)
                  IconButton(
                    visualDensity: VisualDensity.compact,
                    icon: const Icon(Icons.navigation_rounded, color: A.danger),
                    tooltip: 'موقع الزبون على الخريطة',
                    onPressed: () async {
                      if ((t['user_lat'] != null && t['user_lng'] != null)) {
                        openStoreMap(context, t['user_lat'], t['user_lng'], null);
                      } else {
              ...
              const SizedBox(height: 8),
              Row(children: [
                const Icon(Icons.payments_rounded, color: A.success, size: 18),
                const SizedBox(width: 8),
                Text('كاش عند التسليم: ', style: A.t(13)),
                Text(money(total), style: A.t(14, c: A.success, w: FontWeight.w900)),
              ]),
            ]),
          ),
          const SizedBox(height: 14),
          ...
          if (status == 'delivering' && picked)
            SolidBtn(label: 'تم التسليم + قبض الكاش 💵', onTap: () async {
              try {
                await Api.post('/api/delivery/delivered', {'trip_id': t['id']});
      ...
      body: ListView(
        ...
        children: [
          Container(
            ...
            decoration: const BoxDecoration(gradient: A.gradNavy, borderRadius: BorderRadius.all(Radius.circular(22))),
            child: Column(children: [
              const Text('أرباحي من التوصيل', style: TextStyle(color: Colors.white70, fontSize: 13)),
              const SizedBox(height: 6),
              Text(money(balance), style: A.t(30, c: Colors.white, w: FontWeight.w900)),
              const SizedBox(height: 10),
              Text('اليوم: ${money(today)} 💪', style: const TextStyle(color: Color(0xFFBBF7D0), fontSize: 12.5, fontWeight: FontWeight.w800)),
              const SizedBox(height: 14),
              ...
              GlassCard(
                ...
                child: Row(children: [
                  Container(
                    ...
                    decoration: BoxDecoration(color: (t['type'] == 'credit') ? A.success.withOpacity(0.1) : A.danger.withOpacity(0.1), borderRadius: BorderRadius.circular(10)),
